    ...
    if (availableSeats.length > 0) {
        ...
        removeButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (seatItem) {  // ← FIXATO QUI (era seatItem ? .remove())
                    seatItem.remove();
                }
                updateTotalPrice();
            });
...
document.addEventListener('DOMContentLoaded', function() {
    if (!document.getElementById('pageLoadingOverlay')) {
        ...
        overlay.innerHTML = `
            <div class="paw-loader" aria-hidden="true">
                <img src="img/paw-icon-full-color.svg" alt="">
            </div>
        `;
    ...
    function shouldShowOverlay(link) {
        ...
        if (isDownload || target === '_blank') return false;
    ...
    document.addEventListener('click', function(event) {
        ...
        if (!link) return;
        if (!shouldShowOverlay(link)) return;
